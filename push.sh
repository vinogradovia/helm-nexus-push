...
# check jq
jq --version > /dev/null
declare REPO_URL="$(helm repo list --output json | jq --arg REPO "${REPO}" -r '.| map(select(.name == $REPO))| .[].url')"
...
if [[ "${REPO_URL: -1}" != "/" ]]; then REPO_URL+="/"; fi;
...
case "$2" in
    ...
    *)
        ...
        curl --fail -is -u "$AUTH" "$REPO_URL" --upload-file "$CHART_PACKAGE" | indent
        echo "Done"
        ;;
esac
